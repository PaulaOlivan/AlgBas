...
useHash = sys.argv[2] == "1"
test = sys.argv[3] == "1"
imprimir = sys.argv[4] == "1"
...
def separarPalabras(input, real, init = 0):
    ...
    if useHash and init in hash_precalculado:
    ...
    for i in range(init+1, end):
        ...
        if existePalabra(palabra):
            if len(input[i:]) != 0:
                ...
                encontradas = separarPalabras(input, real, i)
                hash_precalculado[i] = [flatten(encontradas).copy()]
                
                if len(encontradas) == 0 and isinstance(encontradas, list):
                    real = "No"
                    break
...
real = "Si"
out = separarPalabras(input, real)
end = time.time()
print("Exec time: ", (end-init), "s")
